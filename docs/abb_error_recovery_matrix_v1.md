...
**I4** (frame-parse failure → skip/abort, abort flavor for R_W_F) and
**I2/I3** (stale-module fix, explicit FIX comments in `TG_Main.mod`)
implemented; **I5 deferred by decision** (FANUC parity — keep documented for
later); **I6 declined**. Implementing I2/I3 surfaced finding **F-E** (§3):
error-handler fall-through is an implicit RETURN, so mid-.tgs errors were
being silently swallowed. VC validation: [robotstudio_setup.md](robotstudio_setup.md)
§11 (I1/I4) and §12 (I2/I3, F-E).
...
| # | Event | Detection | Current behavior | Planned |
|---|-------|-----------|------------------|---------|
| B1 | HMI process dies / closes socket (OS sends RST/FIN) | `ERR_SOCK_CLOSED` on the next send or receive | outside a .tgs run: propagates → `tgMainCycle` → disc → new cycle. Inside a .tgs run: `tgRunTgsProgram`'s handler matched no branch and **fell through — an implicit RETURN, not propagation** (F-E), so the error was silently swallowed (cycle still ended via the normal-flow `TG_SocketDisc`, which is why recovery looked right on the VC) and `UnLoad` never executed (F-B) | **implemented**: handler tail does best-effort UnLoad + explicit `RAISE` (I2/I3) — mid-.tgs aborts now log `TG: cycle error` |
...
| # | Event | Detection | Current behavior | Planned |
|---|-------|-----------|------------------|---------|
| D1 | Module already loaded (Phase-2 manual load) | `ERR_LOADED` | `TRYNEXT` → runs the in-memory module | replace with UnLoad-then-`RETRY` (I2); `TRYNEXT` only if the UnLoad itself fails |
...
| D3 | UnLoad fails | `ERR_UNLOAD` | warn + `TRYNEXT` | keep |
| D4 | PP moved to main by operator | n/a | `\Dynamic` modules are auto-dropped (plan §2.8) — clean by construction | keep |
...
- **F-E — error-handler fall-through is an implicit RETURN, not a RAISE**
  ...
  "propagate to tgMainCycle" — they never did: a mid-.tgs error was silently
  swallowed, the cycle ended through the normal-flow `TG_SocketDisc` (looking
  like a clean recovery), `tgMainCycle`'s `TG: cycle error` line never
  printed, and the module stayed loaded. Fixed by an explicit `tgTryUnload` +
  `RAISE` tail. Behavior change to expect on the VC: mid-.tgs aborts now DO
  print `TG: cycle error, ERRNO = …` ⚠ confirm in §12's kill-mid-cycle test.
  Rule for all future handlers: **never rely on fall-through; end every
  handler branch with RETRY, TRYNEXT, RETURN, or RAISE.**
...
| I4 | Frame parse failure forces skip/abort: `nTG_DoCapture:=0` (R_C_F) / `nTG_WeldStatus:=2` (R_W_F, **abort flavor decided** — safer for welding) after completing the flag exchange (the HMI serves the full request sequence; cutting it short would desync) — HMI sees a normal skip/abort choreography. **Implemented 2026-08-28**; fault-injection knobs `corrupt_cam_frame`/`corrupt_weld_frame` + CLI arg 5 (`corrupt-cam`/`corrupt-weld`) added to `abb_server.py`, 2 fake-robot tests (29 total green) | `TG_ReqCamFrame`, `TG_ReqWeldFrame` | F-C |
